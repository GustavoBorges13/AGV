...
void loop() {
  ...
  //Red:
  if (redValue >= 170 && greenValue <= 150 && blueValue <= 150)
  {
    while (count <= revolution) {
      ...
      count = count + 1;
      delay(2000)
      ligar o motor até que
      chegue no sensor indutivo
      alinhando a direção
    }
  }
  count = 0;
  ...
  //Green:
  if (redValue <= 150 && greenValue >= 170 && blueValue <= 150)
  {
    while (count <= revolution) {
  ...
  //Blue:
  if (redValue <= 200 && greenValue <= 200 && blueValue >= 170)
  {
